...
def get(handler, parameters, url_parameters, *ids_parameters):
    ...
    try:
        r = requests.get(endpoints.DISCORD_GUILD_ROLES.format(*ids_parameters), headers=headers)
        r.raise_for_status()
...
def post(handler, parameters, url_parameters, *ids_parameters):
    ...
    try:
        r = requests.post(endpoints.DISCORD_GUILD_ROLES.format(*ids_parameters), headers=headers, data=parameters)
        r.raise_for_status()
    except requests.exceptions.HTTPError:
