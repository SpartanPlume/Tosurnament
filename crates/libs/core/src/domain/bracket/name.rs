...
pub struct BracketName(String);

use ormlite::postgres::PgArgumentBuffer;
use ormlite::postgres::PgValueRef;
use sqlx::Postgres;

impl sqlx::Type<Postgres> for BracketName {
...
impl sqlx::Encode<'_, Postgres> for BracketName {
    fn encode_by_ref(
        &self,
...
impl sqlx::Decode<'_, Postgres> for BracketName {
...
mod tests {
    ...
    fn empty_name_is_invalid() {
        let name = "".to_string();
        assert_err!(BracketName::try_new(name));
    }
    ...
    fn name_containing_valid_characters_is_valid() {
        let name = "10k-99k".to_string();
        assert_ok!(BracketName::try_new(name));
    }
}
